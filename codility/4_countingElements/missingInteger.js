...
function solution(A) {
  let array = A.filter(el => el > 0);
  if (array.length == 0) return 1;

  array = [...new Set(array)];
  array.sort((a,b)=> a-b);

  let previousInteger = array[0];
  if (previousInteger > 1) return 1;

  for (let i = 1; i < array.length; i++) {
    previousInteger++;
    if(previousInteger < array[i]) break;
  }

  if(previousInteger == array[array.length - 1]) return previousInteger + 1;
